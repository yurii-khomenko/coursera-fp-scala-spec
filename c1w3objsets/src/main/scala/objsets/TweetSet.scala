...
class Tweet(val user: String, val text: String, val retweets: Int) {
  ...
  def max(that: Tweet): Tweet =
    if (retweets > that.retweets) this else that
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet =
    ...
    else if (left.empty) right.mostRetweeted.max(elem)
    else if (right.empty) left.mostRetweeted.max(elem)
    else left.mostRetweeted.max(right.mostRetweeted.max(elem))
